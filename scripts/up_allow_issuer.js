...
// ⬇️ METTI QUI L'ADDRESS DEL NUOVO CONTRATTO DOPO IL DEPLOY
const CONTRACT = "0xE0F24982fA686fEAD94f6b32C532B545c3cEB6CC";
...
async function main() {
  ...
  const calldata = contractIface.encodeFunctionData("setIssuerAllowed", [
    ISSUER_TO_ALLOW,
    true
  ]);
  ...
  // Istanza UP (deve essere controllata dal signer via Key Manager permissions)
  const up = new ethers.Contract(UP_ADMIN, UP_EXEC_ABI, signer);

  console.log("Calling UP.execute(OP_CALL, CONTRACT, 0, calldata) ...");
