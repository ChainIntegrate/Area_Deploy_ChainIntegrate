...
// OpenZeppelin ECDSA (tenuto per compatibilità, puoi anche rimuoverlo se vuoi)
import { ECDSA } from "@openzeppelin/contracts/utils/cryptography/ECDSA.sol";

/**
 * Battery Carbon Footprint Certificate (LSP8) — REV2-A (NO signature check)
 *
 ...
 *
 * REV2-A:
 * - submitContribution NON valida più la firma
 * - l'autenticazione dell'attore è msg.sender == actorByRole[tokenId][role]
 * - digest resta per integrità del file referenziato da uri
 */
contract BatteryCarbonCertificateLSP8_Rev2 is LSP8IdentifiableDigitalAsset {
  using ECDSA for bytes32; // (non usato in REV2-A, ok lasciarlo o rimuoverlo)
  ...
  struct Contribution {
    ...
    string uri;
    bytes32 digest;
    bytes signature; // tenuto per backward-compat / storage schema (opzionale)
    uint64 submittedAt;
    bool exists;
    bool frozen;
    uint64 frozenAt;
  ...
  error InvalidStatus();
  error InvalidRole();
  // error InvalidSignature(); // REV2-A: non serve più (puoi lasciarlo se vuoi compatibilità ABI)
  error AlreadyExists();
  ...
  function submitContribution(
    ...
    string calldata uri,
    bytes32 digest,
    bytes calldata /*signature*/ // REV2-A: ignorata
  ) external {
    _requireCollecting(tokenId);
    ...
    // REV2-A: niente verifica firma. L'identità è msg.sender (UP) + KeyManager permissions.
    c.contributor = actor;
    c.uri = uri;
    c.digest = digest;
    c.signature = ""; // opzionale: svuotiamo sempre per evitare confusione
  ...
  // Nota: lasciata perché magari ti serve off-chain per debug, ma in REV2-A non viene più usata
