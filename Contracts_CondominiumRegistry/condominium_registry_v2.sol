 ...
 */
contract CondominiumRegistryLSP8V2 is LSP8Mintable {
    // =============================================================
    // CONFIG
    // =============================================================
    ...
    mapping(bytes32 => mapping(uint256 => Contractor)) private _contractorsByToken;
    mapping(bytes32 => uint256) public contractorCountByToken;
        ...
        returns (uint256 contractorId)
    {
        ...
        contractorId = ++contractorCountByToken[tokenId];

        _contractorsByToken[tokenId][contractorId] = Contractor({
            id: contractorId,
            tokenId: tokenId,
            name: name,
            walletUP: walletUP,
        ...
        emit ContractorCreated(tokenId, contractorId, name, walletUP);
    }

    function setContractorActive(
        bytes32 tokenId,
    ...
    {
        require(
            _contractorsByToken[tokenId][contractorId].id != 0,
            "Unknown contractor"
        );

        _contractorsByToken[tokenId][contractorId].active = active;

        emit ContractorActiveUpdated(tokenId, contractorId, active);
    }

    function getContractor(
        bytes32 tokenId,
        uint256 contractorId
    ) external view onlyExistingCondominium(tokenId) returns (Contractor memory) {
        require(
            contractorId > 0 && contractorId <= contractorCountByToken[tokenId],
            "Invalid contractorId"
        );
        return _contractorsByToken[tokenId][contractorId];
    }
        ...
        returns (uint256 workId)
    {
        ...
        if (contractorId != 0) {
            ...
            require(
                _contractorsByToken[tokenId][contractorId].active,
                "Inactive contractor"
            );
        }
        ...
        onlyActiveCondominium(tokenId)
    {
        ...
        _addRegistryEventInternal(
            ...
            "contractor",
            _contractorsByToken[tokenId][contractorId].name,
            dataURI,
            dataHash
        );
    }
